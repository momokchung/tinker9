// ck.py Version 3.1.0
__global__
void dfieldgk_cu1(int n, TINKER_IMAGE_PARAMS, real off, const real* restrict x, const real* restrict y,
   const real* restrict z, const Spatial::SortedAtom* restrict sorted, int nakpl, const int* restrict iakpl, int niak,
   const int* restrict iak, const int* restrict lst, real (*restrict fields)[3], real (*restrict fieldps)[3],
   const real* restrict rborn, real gkc, real fc, real fd, real fq)
{
   using d::rpole;
   ...
   for (int ii = ithread; ii < nexclude; ii += blockDim.x * gridDim.x) {
       const int klane = threadIdx.x;    fidsx = 0;fidsy = 0;fidsz = 0;fipsx = 0;fipsy = 0;fipsz = 0;fkdsx = 0;fkdsy =
0;fkdsz = 0;fkpsx = 0;fkpsy = 0;fkpsz = 0;

       int i = exclude[ii][0];
       int k = exclude[ii][1];


       ci[klane] = rpole[i][MPL_PME_0];dix[klane] = rpole[i][MPL_PME_X];diy[klane] = rpole[i][MPL_PME_Y];diz[klane] =
rpole[i][MPL_PME_Z];qixx[klane] = rpole[i][MPL_PME_XX];qixy[klane] = rpole[i][MPL_PME_XY];qixz[klane] =
rpole[i][MPL_PME_XZ];qiyy[klane] = rpole[i][MPL_PME_YY];qiyz[klane] = rpole[i][MPL_PME_YZ];qizz[klane] =
rpole[i][MPL_PME_ZZ];rbi[klane] = rborn[i];xi = x[i];yi = y[i];zi = z[i];xk = x[k];yk = y[k];zk = z[k];ck =
rpole[k][MPL_PME_0];dkx = rpole[k][MPL_PME_X];dky = rpole[k][MPL_PME_Y];dkz = rpole[k][MPL_PME_Z];qkxx =
rpole[k][MPL_PME_XX];qkxy = rpole[k][MPL_PME_XY];qkxz = rpole[k][MPL_PME_XZ];qkyy = rpole[k][MPL_PME_YY];qkyz =
rpole[k][MPL_PME_YZ];qkzz = rpole[k][MPL_PME_ZZ];rbk = rborn[k];
...
if (r2 <= off * off and incl) {
 pair_dfieldgk(r2, xr, yr, zr, gkc, fc, fd, fq,
   ...
   qixx[klane], qixy[klane], qixz[klane], qiyy[klane], qiyz[klane], qizz[klane], rbi[klane],
   ck, dkx, dky, dkz,
   ...
   fkdsx, fkdsy, fkdsz, fkpsx, fkpsy, fkpsz);
       ...
       atomic_add(fidsx, &fields[i][0]);atomic_add(fidsy, &fields[i][1]);atomic_add(fidsz,
&fields[i][2]);atomic_add(fipsx, &fieldps[i][0]);atomic_add(fipsy, &fieldps[i][1]);atomic_add(fipsz,
&fieldps[i][2]);atomic_add(fkdsx, &fields[k][0]);atomic_add(fkdsy, &fields[k][1]);atomic_add(fkdsz,
&fields[k][2]);atomic_add(fkpsx, &fieldps[k][0]);atomic_add(fkpsy, &fieldps[k][1]);atomic_add(fkpsz, &fieldps[k][2]);
   }
   // */
