...
void epolarN2_cu1(int n, CountBuffer restrict nep, EnergyBuffer restrict ep, VirialBuffer restrict vep,
   ...
   const int* restrict iakpl, int niakp, const int* restrict iakp, real (*restrict ufld)[3], real (*restrict dufld)[6],
   const real (*restrict rpole)[10], const real (*restrict uind)[3], const real (*restrict uinp)[3], real f)
{
   using d::jpolar;
   using d::njpolar;
   using d::pdamp;
   ...
   for (int ii = ithread; ii < nexclude; ii += blockDim.x * gridDim.x) {
      ...
      if (r2 <= off * off and incl) {
         ...
         real aewald = 0;
         pair_polar_v2<Ver, NON_EWALD>(r2, xr, yr, zr, scaleb, scalec, scaled, //
   ...
   for (int iw = iwarp; iw < nakpl; iw += nwarp) {
      ...
      for (int j = 0; j < WARP_SIZE; ++j) {
         ...
         if (r2 <= off * off and incl) {
            ...
            real e, vxx, vyx, vzx, vyy, vzy, vzz;
            real aewald = 0;
            pair_polar_v2<Ver, NON_EWALD>(r2, xr, yr, zr, 1, 1, 1, //
               ci[klane], dix[klane], diy[klane], diz[klane], qixx[klane], qixy[klane], qixz[klane], qiyy[klane],
               qiyz[klane], qizz[klane], uidx[klane], uidy[klane], uidz[klane], uipx[klane], uipy[klane], uipz[klane],
               pdi[klane], pga, //
               ck, dkx[threadIdx.x], dky[threadIdx.x], dkz[threadIdx.x], qkxx, qkxy, qkxz, qkyy, qkyz, qkzz,
