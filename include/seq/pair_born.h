...
inline void pair_dgrycuk(real r, real r2, real ri, real rdk, real sk, real mixsn, real pi43, real drbi, real drbpi, real term, bool use_gk, bool useneck, const real* restrict aneck, const real* restrict bneck, const real* restrict rneck, real& de)
{
   if (ri < r+sk) {
      ...
      if (ri+r < sk) {
         lik = sk - r;
         de = de + (real)0.25*pi*(sk2-4*sk*r+17*r2) / (r2*REAL_POW(lik,4));
      }
      else if (r < ri+sk) {
         lik = ri;
         de = de + (real)0.25*pi*(2*ri*ri-sk2-r2) / (r2*REAL_POW(lik,4));
      }
      else {
         lik = r - sk;
         de = de + (real)0.25*pi*(sk2-4*sk*r+r2) / (r2*REAL_POW(lik,4));
      }
      uik = r + sk;
...
inline void tanhrsc (real& ii, real rhoi, real pi43)
{
   ...
   constexpr real recipmaxborn3 = (real)3.70370370370370370370370370370e-5;
   constexpr real b0 = (real)0.9563;
   constexpr real b1 = (real)0.2578;
   constexpr real b2 = (real)0.0810;
...
inline void pair_hctobc(real r, real ri, real rk, real sk, real& pairrborni)
{
   ...
   pairrborni = lik - uik + (real)0.25*r*(uik2lik2) + ((real)0.5/r)*std::log(uik/lik) + ((real)0.25*sk2/r)*(-uik2lik2);
   if (ri < sk-r) {
      pairrborni = pairrborni + 2*(1/ri-lik);
   }
   pairrborni = (real)-0.5 * pairrborni;
}
}
